...
// =====================================================================
// === THUẬT TOÁN 15: MEGA PATTERN BLASTER - NHẬN DIỆN 140+ CẦU VIP ====
// =====================================================================
function algo15_MegaPatternBlaster(history) {
    if (history.length < 20) return null;
    const features = extractFeatures(history);
    ...
    const lastRuns = runs.slice(-10);
    const lengths = lastRuns.map(r => r.len);
    const strLengths = lengths.join('-'); 
    const txStr10 = tx.slice(-10).join('');
    const txStr15 = tx.slice(-15).join('');
    const lastTotals = totals.slice(-5);

    let prediction = null;
    let weightBonus = 1.0;

    // 1. CẦU NHỊP KẾP CHÍNH XÁC (Cầu 1-2-1, 1-3-1, 1-2-3-2-1...)
    const lengthPatterns = {
        "1-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.1 },
        "2-1": { pred: lastTx === 'T' ? 'T' : 'X', weight: 1.1 },
        "3-1": { pred: lastTx === 'T' ? 'T' : 'X', weight: 1.1 },
        "4-1": { pred: lastTx === 'T' ? 'T' : 'X', weight: 1.1 },
        "5-1": { pred: lastTx === 'T' ? 'T' : 'X', weight: 1.1 },
        "1-2": { pred: lastTx, weight: 1.1 },
        "2-2": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.2 },
        "3-2": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.1 },
        "4-2": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.1 },
        "5-2": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.1 },
        "1-3": { pred: lastTx, weight: 1.1 },
        "2-3": { pred: lastTx, weight: 1.1 },
        "3-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.3 },
        "4-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.1 },
        "5-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.1 },
        "1-2-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.5 },
        "2-1-2": { pred: lastTx, weight: 1.5 },
        "1-3-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.4 },
        "3-1-3": { pred: lastTx, weight: 1.4 },
        "2-3-2": { pred: lastTx, weight: 1.4 },
        "3-2-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.4 },
        "1-4-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.3 },
        "4-1-4": { pred: lastTx, weight: 1.3 },
        "2-4-2": { pred: lastTx, weight: 1.3 },
        "4-2-4": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.3 },
        "3-4-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.3 },
        "4-3-4": { pred: lastTx, weight: 1.3 },
        "1-2-2-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.6 },
        "2-1-1-2": { pred: lastTx, weight: 1.6 },
        "1-3-3-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.5 },
        "3-1-1-3": { pred: lastTx, weight: 1.5 },
        "2-3-3-2": { pred: lastTx, weight: 1.5 },
        "3-2-2-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.5 },
        "1-2-3-2-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 2.0 },
        "2-1-3-1-2": { pred: lastTx, weight: 2.0 },
        "3-2-1-2-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 2.0 },
        "1-3-2-3-1": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.8 },
        "2-3-1-3-2": { pred: lastTx, weight: 1.8 },
        "3-1-2-1-3": { pred: lastTx === 'T' ? 'X' : 'T', weight: 1.8 }
    };

    // Tìm quy luật chuỗi độ dài phù hợp nhất (dài nhất ưu tiên)
    let bestSeqMatch = "";
    for (const seq in lengthPatterns) {
        if (strLengths.endsWith(seq) && seq.length > bestSeqMatch.length) {
            bestSeqMatch = seq;
        }
    }
    if (bestSeqMatch) {
        prediction = lengthPatterns[bestSeqMatch].pred;
        weightBonus = lengthPatterns[bestSeqMatch].weight;
    }

    // 2. CẦU SO GƯƠNG / ĐỐI XỨNG (Cầu đối xứng, Cầu phản chiếu, Cầu soi gương)
    if (!prediction) {
        const checkSymmetry = (len) => {
            const part = tx.slice(-len).join('');
            return part === part.split('').reverse().join('');
        };
        if (checkSymmetry(9)) { prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.9; } // Cầu soi gương 4 nhịp
        else if (checkSymmetry(7)) { prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.7; } // Cầu soi gương 3 nhịp
        else if (checkSymmetry(5)) { prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.5; } // Cầu soi gương 2 nhịp
    }

    // 3. CẦU HÌNH THÁI CHUỖI KÝ TỰ (Cầu xen kẽ, Cầu kẹp, Cầu tam/tứ/ngũ)
    if (!prediction) {
        if (txStr10.endsWith("TXTXTXTX") || txStr10.endsWith("XTXTXTXT")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.6; // Cầu 1-1 dài (Cầu đảo 1-1 / Cầu zigzag)
        } else if (txStr10.endsWith("TTXXTTXX") || txStr10.endsWith("XXTTXXTT")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.5; // Cầu 2-2 (Cầu xen đôi / kép)
        } else if (txStr10.endsWith("TTTXXXTTT") || txStr10.endsWith("XXXTTTXXX")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.6; // Cầu 3-3 (Cầu tam)
        } else if (txStr10.endsWith("TTTTXXXX") || txStr10.endsWith("XXXXTTTT")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.5; // Cầu tứ
        } else if (txStr15.includes("TTTTT") && txStr10.endsWith("X")) {
            prediction = 'X'; weightBonus = 1.4; // Cầu phá bệt / Né bệt
        } else if (txStr10.endsWith("TXT") || txStr10.endsWith("XTX")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.2; // Cầu kẹp 1 / Kẹp tâm
        } else if (txStr10.endsWith("TXXT") || txStr10.endsWith("XTTX")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.3; // Cầu kẹp 2 / Đảo kẹp
        } else if (txStr10.endsWith("TXXXT") || txStr10.endsWith("XTTTX")) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.4; // Cầu kẹp 3
        }
    }

    // 4. CẦU HÌNH THÁI XU HƯỚNG TỔNG ĐIỂM (Cầu bậc thang, Sóng chữ M/W/Z)
    if (!prediction) {
        const isUpward = lastTotals[0] < lastTotals[1] && lastTotals[1] < lastTotals[2] && lastTotals[2] < lastTotals[3] && lastTotals[3] < lastTotals[4];
        const isDownward = lastTotals[0] > lastTotals[1] && lastTotals[1] > lastTotals[2] && lastTotals[2] > lastTotals[3] && lastTotals[3] > lastTotals[4];
        
        if (isUpward) { prediction = 'T'; weightBonus = 1.5; } // Cầu bậc thang tăng / Leo núi / Tăng đều
        else if (isDownward) { prediction = 'X'; weightBonus = 1.5; } // Cầu bậc thang giảm / Tụt dốc / Giảm đều
        else if ((lastTotals[0] < lastTotals[1] && lastTotals[1] > lastTotals[2] && lastTotals[2] < lastTotals[3] && lastTotals[3] > lastTotals[4])) {
            prediction = lastTx === 'T' ? 'T' : 'X'; weightBonus = 1.3; // Cầu chữ M / Chữ W / Sóng ngắn / Nhịp 2
        } else if ((lastTotals[0] > lastTotals[1] && lastTotals[1] < lastTotals[2] && lastTotals[2] > lastTotals[3] && lastTotals[3] < lastTotals[4])) {
            prediction = lastTx === 'T' ? 'X' : 'T'; weightBonus = 1.3; // Cầu chữ Z / Sóng ba
        }
    }

    // 5. NHÓM CẦU HỒI QUY VÀ CHẶN BỆT (Xác suất, Lọc nhiễu)
    if (!prediction && runs.length >= 3) {
        const lastRunLen = runs[runs.length - 1].len;
        const prevRunLen = runs[runs.length - 2].len;
        
        if (lastRunLen >= 6) { prediction = lastTx; weightBonus = 2.0; } // Cầu bệt Tài/Xỉu (Ép Tài/Ép Xỉu)
        else if (lastRunLen === 1 && prevRunLen >= 5) { prediction = lastTx === 'T' ? 'T' : 'X'; weightBonus = 1.4; } // Cầu hồi 1 / Gãy giả
        else if (lastRunLen === 2 && prevRunLen >= 5) { prediction = lastTx === 'T' ? 'T' : 'X'; weightBonus = 1.3; } // Cầu hồi 2
        else if (lastRunLen === 3 && prevRunLen >= 5) { prediction = lastTx === 'T' ? 'T' : 'X'; weightBonus = 1.2; } // Cầu hồi 3
    }
...
// =====================================================================
// === THUẬT TOÁN 16: ULTIMATE 300 MASTER (Tích hợp 300 Cầu File Ntdat) =
// =====================================================================
function algo16_Ultimate300Master(history) {
    if (history.length < 20) return null;
    const features = extractFeatures(history);
    const { runs, tx, totals } = features;
    ...
    const txStr = tx.join('');
    const last10Tx = txStr.slice(-10);
    const last15Tx = txStr.slice(-15);
    const lastTx = tx[tx.length - 1];
    
    const recentRuns = runs.slice(-10);
    const runStr = recentRuns.map(r => r.len).join('-');
    const lastRunLen = runs[runs.length - 1].len;
    
    let prediction = null;
    
    // --- 1. KIỂM TRA DẤU HIỆU "SẮP BẺ" TRƯỚC (QUAN TRỌNG NHẤT TRONG FILE TEXT) ---
    // (Rules 161-170: Bẻ thật, Bẻ giả; Rules 171-220: Báo sớm, Nhiễu)
    
    // Đang bệt dài (>= 7) mà tổng điểm tự nhiên giảm mạnh -> Phá bệt (Rule 16)
    if (lastRunLen >= 7) {
        const last3Totals = totals.slice(-3);
        if (last3Totals[0] > last3Totals[1] && last3Totals[1] > last3Totals[2]) {
            return lastTx === 'T' ? 'X' : 'T'; // Phá bệt
        }
        return lastTx; // Chưa có dấu hiệu bẻ thì ôm bệt (Rule 16)
    }

    // Đang đi 1-1 dài mà xuất hiện TT hoặc XX (Rule 1: Dấu hiệu sắp bẻ)
    if (runStr.endsWith('1-1-1-1-1-2')) {
        return lastTx; // Dự kiến chuyển sang 2-1 hoặc Bệt 2
    }
    
    // Đang đi 2-1 dài mà X kéo dài thành XX (Rule 2: Dấu hiệu sắp bẻ)
    if (runStr.endsWith('2-1-2-1-2-2')) {
        return lastTx === 'T' ? 'X' : 'T'; // Chuyển sang 2-2
    }

    // Đang đi 3-1 dài mà xuất hiện TTTT (Rule 3) -> chuyển 4-1
    if (runStr.endsWith('3-1-3-1-4')) {
        return lastTx === 'T' ? 'X' : 'T'; 
    }

    // Gãy 2 liên tiếp khi đang đi 4-1 (Rule 4)
    if (runStr.endsWith('4-1-4-2')) {
        return lastTx === 'T' ? 'X' : 'T'; // Chuyển 4-2
    }

    // --- 2. NHẬN DIỆN CÁC THẾ CẦU ĐỐI XỨNG / SOI GƯƠNG / CHỮ M/W/V (Rules 32-37, 51-52, 125-129) ---
    // Cầu chữ M (Rule 33): Tổng điểm lên đỉnh -> xuống -> lên đỉnh thấp hơn -> dự đoán xuống tiếp
    const tLen = totals.length;
    if (tLen >= 5) {
        const t = totals.slice(-5);
        if (t[0] < t[1] && t[1] > t[2] && t[2] < t[3] && t[3] > t[2] && t[3] < t[1]) {
            prediction = lastTx === 'T' ? 'X' : 'T'; // Đi xuống tạo chân chữ M
        }
        // Cầu chữ W (Rule 34): Xuống đáy -> lên -> xuống đáy nông hơn -> lên tiếp
        if (t[0] > t[1] && t[1] < t[2] && t[2] > t[3] && t[3] < t[2] && t[3] > t[1]) {
            prediction = lastTx === 'T' ? 'X' : 'T'; // Bật lên
        }
    }

    if (prediction) return prediction;

    // Soi gương 2,3 nhịp (Rule 128, 129)
    for (let mirrorLen of [6, 8, 10]) {
        if (last15Tx.length >= mirrorLen) {
            const firstHalf = last15Tx.slice(-mirrorLen, -mirrorLen/2);
            const secondHalf = last15Tx.slice(-mirrorLen/2);
            const reversedFirst = firstHalf.split('').reverse().join('');
            if (secondHalf.slice(0, secondHalf.length-1) === reversedFirst.slice(0, secondHalf.length-1)) {
                prediction = reversedFirst[secondHalf.length - 1]; // Đánh theo nhịp gương
                break;
            }
        }
    }
    ...
    // --- 3. KIỂM TRA 140+ MẪU CHUỖI NHỊP CHUẨN TỪ 101-150 ---
    // Áp dụng so khớp trực tiếp dãy nhịp (runs)
    const nhipVIP = {
        '1-2-1-2': { pred: lastTx === 'T' ? 'X' : 'T' }, // Kéo tiếp 1-2-1
        '2-1-2-1': { pred: lastTx === 'T' ? 'X' : 'T' },
        '1-3-1': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 103
        '3-1-3': { pred: lastTx }, // Rule 104
        '2-3-2': { pred: lastTx }, // Rule 105
        '3-2-3': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 106
        '1-4-1': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 107
        '4-1-4': { pred: lastTx }, // Rule 108
        '2-4-2': { pred: lastTx }, // Rule 109
        '4-2-4': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 110
        '1-2-2-1': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 113
        '2-1-1-2': { pred: lastTx }, // Rule 114
        '1-2-3-2-1': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 119
        '2-1-3-1-2': { pred: lastTx }, // Rule 120
        '3-2-1-2-3': { pred: lastTx === 'T' ? 'X' : 'T' }, // Rule 121
    };

    for (let pattern in nhipVIP) {
        if (runStr.endsWith(pattern)) {
            return nhipVIP[pattern].pred;
        }
    }

    // --- 4. CÁC CẦU KẸP / XEN KẼ (Rules 53-60) ---
    if (last10Tx.endsWith('XTX')) return 'X'; // Kẹp 1 (Rule 53)
    if (last10Tx.endsWith('TXT')) return 'T';
    if (last10Tx.endsWith('XXTTXX')) return 'T'; // Kẹp 2 (Rule 54)
    if (last10Tx.endsWith('TTXXTT')) return 'X';
    
    // Xen kẽ kéo dài (Rule 56)
    if (last10Tx.endsWith('TXTXTX')) return 'T'; 
    if (last10Tx.endsWith('XTXTXT')) return 'X';

    // --- 5. LỌC NHIỄU & CẦU ĐỨT ĐOẠN (Rules 68, 91, 206-210) ---
    // Khi nhịp loạn (run.len thay đổi liên tục 1,3,2,4,1) -> Đánh theo xu hướng thuận của tổng điểm
    const last5Runs = recentRuns.slice(-5).map(r => r.len);
    const setRuns = new Set(last5Runs);
    if (setRuns.size >= 4) { // Nhịp rất loạn (Nhiễu mạnh)
        const mean5 = avg(totals.slice(-5));
        return mean5 >= 11 ? 'T' : 'X'; // Đánh theo lực rơi tổng MD5
    }

    // --- 6. CẦU CHẠY ĐỀU / CHẠY SỐC (Rules 81, 82) ---
    // Tổng điểm giữ mức đều (10, 11, 10, 12, 11) -> Cầu trung bình
    const last3T = totals.slice(-3);
    const variance3 = avg(last3T.map(t => Math.pow(t - avg(last3T), 2)));
    if (variance3 < 1.5) {
        // Đang rất đều, đánh theo nhịp cũ
        return lastTx === 'T' ? 'X' : 'T'; 
    } else if (variance3 > 6) {
        // Chạy sốc, bẻ gắt
        return lastTx; 
...
// --- DANH SÁCH THUẬT TOÁN ĐẦY ĐỦ (Đã thêm Algo 16: Nhận diện 300+ Cầu) ---
const ALL_ALGS = [
    ...
    { id: 'algo15_mega_pattern_blaster', fn: algo15_MegaPatternBlaster },
    { id: 'algo16_ultimate_300_master', fn: algo16_Ultimate300Master } // BỘ MÁY XỬ LÝ 300 QUY TẮC CẦU TỪ FILE NTDAT
];
    ...
    predict(history) {
        ...
        for (const a of this.algs) {
            try {
                ...
                // Gia tăng trọng số đặc biệt cho các thuật toán VIP
                if (a.id === 'k_vip_master_pattern') weight *= 1.5;
                if (a.id === 'algo14_exact_history_match') weight *= 1.6; 
                if (a.id === 'algo15_mega_pattern_blaster') weight *= 1.8;
                // Trọng số TỐI ĐA (x2.5) cho hệ thống Ultimate 300 Master mới tích hợp
                if (a.id === 'algo16_ultimate_300_master') weight *= 2.5; 
...
function getComplexPattern(history) {
    ...
    // Tích hợp tên Cầu từ hệ thống 300 Mẫu Ntdat (Algo 16 nhận diện)
    let ntdat300Name = "";
    if (algo16_Ultimate300Master(history) !== null) {
        ntdat300Name = " [Khớp Mẫu Chuẩn 300 Cầu]";
    }

    if (vipPat && vipPat.length > 0) {
        const vnNames = vipPat.map(vp => Object.keys(VIP_PATTERN_MAP).find(k => VIP_PATTERN_MAP[k] === vp) || vp);
        return `[VIP HOÀNG: ${vnNames.join(', ')}]${ntdat300Name} - ${baseStr}`;
    }
    return baseStr + ntdat300Name;
}
...
app.get("/", async () => { 
    return {
        ...
        version: "9.0 SUPER MEGA VIP (Đã tích hợp 300+ Thuật Toán Cầu Chuyên Sâu từ file Ntdat)", 
        algorithms: ALL_ALGS.length,
        pattern_recognition: "VIP Hoàn Chỉnh (Tích hợp Cầu Hình Chữ M/W, Phản Chiếu, Gãy Giả, Chuyển Pha, Báo Sớm)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
const start = async () => {
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro V9.0 - SUPER MEGA VIP HOÀNG Đã Khởi Động!");
    ...
    console.log(`\n🔧 Hệ thống AI VIP với ${ALL_ALGS.length} thuật toán nguyên bản + Data tĩnh mới:`);
    ALL_ALGS.forEach((alg, i) => console.log(`   ${i+1}. ${alg.id}`));
    console.log("\n🎯 CẬP NHẬT GIA TRỌNG MỚI (V9.0 SUPER MEGA VIP):");
    console.log("   • [THUẬT TOÁN 16]: Tích hợp thành công 300 Cầu siêu cấp từ (300ttoanNtdatLc79.txt).");
    console.log("   • [GIỮ NGUYÊN CODE LÕI]: Tất cả các thuật toán cũ từ 1-15 chạy bình thường không mất bất cứ thứ gì.");
    console.log("   • Hệ thống AI giờ đây có thể tự tính toán Điểm Gãy, Nhịp Bẻ, Cầu Soi Gương, và Hình thái Chữ cái (M, W, V).");
};
start();